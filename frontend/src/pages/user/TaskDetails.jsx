...
const TaskDetails = () => {
  ...
  const getTaskInfo = async () => {
    try {
      const response = await apiService.get(API_PATHS.TASKS.GET_TASK_BY_ID(id));
      if (response.data) {
        ...
        {task && (
          <div className="grid grid-cols-1 md:grid-cols-4 mt-4">
            <div className="form-card col-span-3">
              ...
              <div className="mt-4">
                <InfoBox title="Description" value={task?.description} />
              </div>

              <div className="grid grid-cols-12 gap-4 mt-4">
                <div className="col-span-6 md:col-span-4">
                  <InfoBox tittle="Priority" value={task?.priority} />
                </div>
                <div className="col-span-6 md:col-span-4">
                  <InfoBox
                    tittle="Due Date"
                ...
                <div className="mt-2">
                  ...
                  {task?.attachments?.map((link, index) => (
                    <Attachment
                      ...
                      link={link}
                      index={index}
                      onClick={() => handleAttachmentClick(link)}
                    />
                  ))}
                </div>
              )}
...
const TodoCheckList = ({ text, isChecked, onChange }) => {
  return (
    <div className="flex items-center gap-3 p-3">
      ...
      />
      <p className="text-[12px] md:text[13px] font-medium text-gray-800">
        {text}
      </p>
    </div>
  );
